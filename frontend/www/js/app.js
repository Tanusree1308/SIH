// --- CONFIGURATION ---
const API_BASE_URL = 'http://10.0.47.7:8000'; // IMPORTANT: Make sure this port matches your running server

// --- TRANSLATIONS ---
// ... (all your existing translation code is unchanged) ...
const translations = {
    en: {
...
// --- API FUNCTIONS ---
// ... (all your existing API functions are unchanged) ...
...
        console.log('Attempting to fetch from URL:', `${API_BASE_URL}/analysis/`);
...
// --- NEW FUNCTION: Analyze image data from Capacitor Camera ---
// This function takes image data (as a blob) and sends it for analysis,
// reusing your existing API logic.
async function analyzeCapturedImage(blob) {
    ...
    try {
        ...
        if (!response.ok) {
            alert(`Analysis failed: ${result.detail}`);
            showScreen('dashboard-screen'); // Go back to dashboard on failure
        } else {
            populateResults(result);
            showScreen('results-screen');
        }
        ...
        console.error('Analysis error:', error);
        alert("An error occurred during analysis. Please ensure the backend is running.");
        showScreen('dashboard-screen'); // Go back to dashboard on error
    }
}


// --- NEW FUNCTION: Open native camera/gallery using Capacitor ---
// This function handles opening the camera or photo gallery and then
// calls the analysis function.
async function getPhotoFromNative(source) {
    // Check if Capacitor Camera plugin is available
    if (!window.Capacitor || !window.Capacitor.Plugins.Camera) {
        console.error("Capacitor Camera plugin not available.");
        // Fallback for web - could redirect to the old upload screen
    ...
    try {
        const image = await Camera.getPhoto({
            quality: 90,
            allowEditing: false,
            resultType: CameraResultType.Uri, // Use Uri for better performance, convert to blob later
        ...
        // Now send this blob for analysis
        await analyzeCapturedImage(blob);

    } catch (error) {
        // This error is common if the user cancels the camera or photo selection.
        // We can safely ignore it without showing an alert.
...
// --- CORE FUNCTIONS ---
// ... (all your existing core functions are unchanged) ...
...
// --- MODIFIED --- Event listener for the "Upload Photo" button
document.getElementById('go-to-upload-btn')?.addEventListener('click', (e) => { 
    e.preventDefault(); 
    // Check if we are in the native app
    if (window.Capacitor && window.Capacitor.isNativePlatform()) {
        // If yes, use the Capacitor plugin to open the photo gallery
        getPhotoFromNative('PHOTOS');
    } else {
        // If no (we're in a browser), show the original upload screen
        showScreen('upload-screen'); 
    }
});

// --- MODIFIED --- Event listener for the "Scan Photo" button
document.getElementById('go-to-camera-btn')?.addEventListener('click', (e) => { 
    e.preventDefault(); 
    // Check if we are in the native app
    if (window.Capacitor && window.Capacitor.isNativePlatform()) {
        // If yes, use the Capacitor plugin to open the camera directly
        getPhotoFromNative('CAMERA');
    } else {
        // If no (we're in a browser), show the original screen with the live camera feed
